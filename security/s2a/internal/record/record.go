...
// Read reads and decrypts a record from the underlying connection, and copies
// the decrypted payload into b. If the size of the payload is greater than
// len(b), Read retains the remaining bytes in an internal buffer, and
// subsequent calls to Read will read from this buffer until it is exhausted.
...
func (p *conn) Read(b []byte) (n int, err error) {
	if len(p.pendingApplicationData) == 0 {
		...
		record, err := p.readCompletedRecord()
		if err != nil {
			return 0, err
		}
		// Now we have a complete record, so split the header and validate it.
		// [payload] = [ciphertext of application data] + [ciphertext of record
		// type byte] + [(optionally) ciphertext of padding by zeros] + [tag]
		...
		// Remove the padding by zeros and the record type byte.
		...
		case handshake:
			...
			if handshakeMsgType == tlsHandshakeKeyUpdatePrefix {
				...
				fmt.Println("Session ticket was received")
				return 0, nil
			}
...
// stripPaddingAndType strips the padding by zeros and record type from a
// record and returns the record type. Note that p.pendingApplicationData
// should be of the form [application data] + [record type byte] +
// [trailing zeros]
func (p *conn) stripPaddingAndType() (recordType, error) {
	...
	ct := recordType(p.pendingApplicationData[i])
...
// splitAndValidateHeader splits the header from the payload in the completed
...
func splitAndValidateHeader(completedRecord []byte) (header, payload []byte, err error) {
	if len(completedRecord) < tlsRecordHeaderSize {
		return nil, nil, fmt.Errorf("record was smaller than the header size")
	}
	header = completedRecord[:tlsRecordHeaderSize]
	payload = completedRecord[tlsRecordHeaderSize:]
