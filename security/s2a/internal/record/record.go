...
// until it is exhausted. Note that for the user to efficiently call this
// method, the user should ensure that the buffer b is allocated such that the
// buffer does not have any unused segments. This can be done by calling Read
// via io.ReadFull, which continually calls Read until the specified buffer has
// been filled. Also note that the user should close the connection via Close()
// if an error is thrown by a call to Read.
func (p *conn) Read(b []byte) (n int, err error) {
	...
	if len(p.pendingApplicationData) == 0 {
		...
		// + [(optionally) ciphertext of padding by zeros] + [tag]
...
// parseRecord parses the provided buffer and returns a completed record and any
...
// not be greater than maxLen, otherwise an error is returned.
func parseRecord(b []byte, maxLen uint16) (completedRecord, remaining []byte, err error) {
