...
func TestConnNewSessionTicket(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if got, want := newConn.ticketRemainingSize, uint32(0); got != want {
				t.Errorf("newConn.ticketRemainingSize = %v, want %v", got, want)
			}
			if got, want := newConn.ticketBuf, tc.outTicketBuf; !bytes.Equal(got, want) {
				t.Errorf("newConn.ticketBuf = %v, want %v", got, want)
			}
		})
	}
}
...
func TestConnNewSessionTicketWithTicketBuilder(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			finalTicketState: ticketReceiving,
			outTicketBuf:     make([]byte, tlsRecordMaxPlaintextSize/2+tlsRecordMaxPlaintextSize),
		},
		{
		...
		},
		{
			...
			finalTicketState: ticketReceiving,
			outTicketBuf:     make([]byte, tlsRecordMaxPlaintextSize*2),
		},
		{
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if got, want := c.ticketRemainingSize, uint32(0); got != want {
				t.Errorf("newConn.ticketRemainingSize = %v, want %v", got, want)
			}
			if got, want := c.ticketBuf, tc.outTicketBuf; !bytes.Equal(got, want) {
				t.Errorf("newConn.ticketBuf = %v, want %v", got, want)
			}
		})
	}
