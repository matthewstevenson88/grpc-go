...
func TestExpand(t *testing.T) {
	...
	// slightly modified to fit our interface and implementation.
	for _, tc := range []struct {
	...
	}{
		{
			...
			okm:  testutil.Dehex("3cb25f25faacd57a90434f64d0362f2a2d2d0a90cf1a5a4c5db02d56ecc4c5bf"),
		},
		{
			...
			okm:  testutil.Dehex("b11e398dc80327a1c8e7f78c596a49344f012eda2d4efad8a050cc4c19afa97c"),
		},
		{
			desc: "sha256 zero length info",
			prk:  testutil.Dehex("19ef24a32c717b167f33a91d6f648bdf96596776afdb6377ac434c1c293ccb04"),
			okm:  testutil.Dehex("8da4e775a563c18f715f802a063c5a31b8a11f5c5ee1879ec3454e5f3c738d2d"),
		},
	} {
		t.Run(tc.desc, func(t *testing.T) {
			expander := newDefaultHKDFExpander()
			got, err := expander.expand(sha256.New, tc.prk, tc.info)
			if err != nil {
				t.Errorf("expand failed with error: %v", err)
			}
			if !bytes.Equal(got, tc.okm) {
				t.Errorf("expand(sha256.New, %v, %v) = %v, want %v.", tc.prk, tc.info, got, tc.okm)
			}
		})
	}
}
