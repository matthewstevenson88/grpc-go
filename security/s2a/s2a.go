...
type s2aTransportCreds struct {
	...
	// localIdentity should only be used by the server.
	localIdentity *s2apb.Identity
	// localIdentities should only be used by the client.
...
func NewClientCreds(opts *ClientOptions) (credentials.TransportCredentials, error) {
	...
	minTLSVersion, err := toProtoTLSVersion(opts.MinTLSVersion)
	if err != nil {
		return nil, err
	}
	maxTLSVersion, err := toProtoTLSVersion(opts.MaxTLSVersion)
	if err != nil {
		return nil, err
	}
