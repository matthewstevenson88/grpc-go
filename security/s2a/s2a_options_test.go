...
func TestToProtoIdentity(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			identity:    NewSpiffeID("test spiffe id"),
			outIdentity: &s2apb.Identity{IdentityOneof: &s2apb.Identity_SpiffeId{SpiffeId: "test spiffe id"}},
		},
		{
			identity:    NewHostname("test hostname"),
			outIdentity: &s2apb.Identity{IdentityOneof: &s2apb.Identity_Hostname{Hostname: "test hostname"}},
		},
	} {
